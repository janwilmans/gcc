...
void CheckNoDeepBind(const char *filename, int flag) {
  ...
  if (flag & RTLD_DEEPBIND) {
    Report(
        ...
        "(see https://github.com/google/sanitizers/issues/611 and "
        "https://github.com/janwilmans/gcc/issues/1 for details"
        "). If you want to run %s library under sanitizers please remove "
        "RTLD_DEEPBIND from dlopen flags.\n(Patched to continue anyway by Jan Wilmans)\n",
        filename, filename);
    // Die(); // commented, this is the only functional change.
  }
#endif
}
